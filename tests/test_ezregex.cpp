...
TEST_CASE("exactly EZ_REGEX_MAX_CAPTURES groups all captured") {
    std::string pat, str;
    for (int i = 0; i < EZ_REGEX_MAX_CAPTURES; ++i) { pat += "(a)"; str += "a"; }
    std::vector<std::string_view> caps;
...
TEST_CASE("exceeding EZ_REGEX_MAX_CAPTURES returns EZ_REGEX_ERR_DEPTH") {
    std::string pat, str;
    for (int i = 0; i < EZ_REGEX_MAX_CAPTURES + 1; ++i) { pat += "(a)"; str += "a"; }
    std::vector<std::string_view> caps;
...
TEST_CASE("error: nested groups returns EZ_REGEX_ERR_NESTING") {
    ...
    CHECK(_test_validate_pattern("(a)(b(c))")== EZ_REGEX_ERR_NESTING);  // nested in second group
...
TEST_CASE("error: too many capture groups returns EZ_REGEX_ERR_DEPTH") {
    ...
    std::string pat;
    for (int i = 0; i <= EZ_REGEX_MAX_CAPTURES; ++i) pat += "(a)";
...
TEST_CASE("ISO date with captures: year, month, day extracted") {
    std::vector<std::string_view> caps;
    REQUIRE(ez::regex_match("^(\\d{4})-(\\d{2})-(\\d{2})$", "2024-01-15", &caps) == EZ_REGEX_MATCH);
    REQUIRE(caps.size() == 3);
    CHECK(caps[0] == "2024");
    CHECK(caps[1] == "01");
    CHECK(caps[2] == "15");
...
TEST_CASE("key=value found inside a larger string") {
    std::vector<std::string_view> caps;
    REQUIRE(ez::regex_match("(\\w+)=(\\w+)", "config: timeout=30 retries=3", &caps) == EZ_REGEX_MATCH);
    REQUIRE(caps.size() == 2);
    CHECK(caps[0] == "timeout");
    CHECK(caps[1] == "30");
}
...
TEST_CASE("email-like pattern extracts user and domain") {
    std::vector<std::string_view> caps;
    REQUIRE(ez::regex_match("^([\\w]+)@([\\w]+)\\.([a-z]+)$", "alice@example.com", &caps) == EZ_REGEX_MATCH);
    REQUIRE(caps.size() == 3);
    CHECK(caps[0] == "alice");
...
TEST_CASE("(\\w+) repeated pattern: captures leftmost greedy token") {
    ...
    std::vector<std::string_view> caps;
    REQUIRE(ez::regex_match("^(\\w+)\\s(\\w+)\\s(\\w+)$", "one two three", &caps) == EZ_REGEX_MATCH);
    REQUIRE(caps.size() == 3);
    CHECK(caps[0] == "one");
    CHECK(caps[1] == "two");
    CHECK(caps[2] == "three");
...
TEST_CASE("long option: --write") {
    ...
    REQUIRE(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)$", "--verbose",     &caps) == EZ_REGEX_MATCH);
    CHECK(caps[0] == "verbose");

    REQUIRE(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)$", "--output-file", &caps) == EZ_REGEX_MATCH);
    CHECK(caps[0] == "output-file");

    REQUIRE(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)$", "--opt123",      &caps) == EZ_REGEX_MATCH);
    CHECK(caps[0] == "opt123");
    ...
    CHECK(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)$", "-write",      &caps) == EZ_REGEX_NO_MATCH);  // single dash
    CHECK(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)$", "--",          &caps) == EZ_REGEX_NO_MATCH);  // no name
    CHECK(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)$", "--123",       &caps) == EZ_REGEX_NO_MATCH);  // starts with digit
    CHECK(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)$", "--write=val", &caps) == EZ_REGEX_NO_MATCH);  // has value
    CHECK(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)$", "write",       &caps) == EZ_REGEX_NO_MATCH);  // no dashes
}
...
TEST_CASE("long option with value: --user=name") {
    ...
    REQUIRE(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)=(\\S+)$", "--user=alice",            &caps) == EZ_REGEX_MATCH);
    REQUIRE(caps.size() == 2);
    CHECK(caps[0] == "user");
    CHECK(caps[1] == "alice");

    REQUIRE(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)=(\\S+)$", "--count=42",              &caps) == EZ_REGEX_MATCH);
    CHECK(caps[0] == "count");
    CHECK(caps[1] == "42");

    REQUIRE(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)=(\\S+)$", "--output-file=/tmp/out",  &caps) == EZ_REGEX_MATCH);
    CHECK(caps[0] == "output-file");
    CHECK(caps[1] == "/tmp/out");

    REQUIRE(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)=(\\S+)$", "--level=debug",           &caps) == EZ_REGEX_MATCH);
    CHECK(caps[0] == "level");
    CHECK(caps[1] == "debug");

    CHECK(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)=(\\S+)$", "--user=",      &caps) == EZ_REGEX_NO_MATCH);  // empty value
    CHECK(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)=(\\S+)$", "--user alice", &caps) == EZ_REGEX_NO_MATCH);  // space not =
    CHECK(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)=(\\S+)$", "-user=alice",  &caps) == EZ_REGEX_NO_MATCH);  // single dash
    CHECK(ez::regex_match("^--([a-zA-Z][a-zA-Z0-9-]*)=(\\S+)$", "--=alice",     &caps) == EZ_REGEX_NO_MATCH);  // missing name
}
