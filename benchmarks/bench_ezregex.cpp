...
static void bench_group(const char* title, const char* pat, const std::string& input,
                        StdMatchFn std_match, uint64_t min_iters = 25000)
{
    ...
    nb::Bench()
        ...
        .run("std::regex hot    ", [&] { nb::doNotOptimizeAway(std_match(re_hot, m, input)); })
        .run("std::regex cold   ", [&] {
...
int main()
{
    ...
    bench_group("1. ISO date  ^\\d{4}-\\d{2}-\\d{2}$", R"(^\d{4}-\d{2}-\d{2}$)", s_date, do_match);

    // ── 2. Key=value — anchored full match, 2 captures ────────────────────────
    bench_group("2. Key=value  ^(\\w+)=(\\w+)$", R"(^(\w+)=(\w+)$)", s_kv, do_match);

    // ── 3. Digit extraction — substring search, 1 capture ─────────────────────
    bench_group("3. Digit extraction  \\d+  in short string", R"(\d+)", s_digits, do_search);

    // ── 4. Email — anchored full match, 3 captures ────────────────────────────
    bench_group("4. Email  ^([\\w]+)@([\\w]+)\\.([a-z]+)$", R"(^([\w]+)@([\w]+)\.([a-z]+)$)",
                s_email, do_match);
    ...
    bench_group("5. Long string (10 000 chars)  needle near end", "needle", s_long, do_search,
                /*min_iters=*/500);

    // ── 6. No-match exhaustion — full scan, no match (1 000 chars) ────────────
    // std::regex takes ~430 µs each; 200 iters keeps total runtime reasonable.
    bench_group("6. No-match exhaustion  \\d{6}  on 1000-char alpha string", R"(\d{6})", s_nomatch,
                do_search,
                /*min_iters=*/200);
